module.exports = {
    ...
    getListingImages: function(req, res, next) {
        ...
        db.get_listing_images([listingId]).then(listingImages => {
    ...
    addListing: function(req, res, next) {
        ...
        db.add_listing([
            ...
            db.add_listing_image([
                response[0].listing_id,
                image1,
                image2,
                image3
            ])
    ...
    getUserListings: function(req, res, next) {
        ...
        db.get_user_listings([userid]).then(listings => {
            res.status(200).send(listings);
        });
    },
    ...
    deleteListing: function(req, res, next) {
        ...
        db.delete_listing_images([listingId]).then(response => {
            db.remove_grail([listingId, userId]).then(response => {
                db.delete_listing([listingId, userId]).then(listings => {
                    res.status(200).send(listings);
                });
            });
        });
